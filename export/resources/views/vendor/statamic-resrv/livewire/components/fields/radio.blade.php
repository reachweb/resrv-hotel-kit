...
<div {{ $attributes->class(['relative', 'md:col-span-2' => $field['width'] === 100, 'md:col-span-1' => $field['width'] === 50,]) }} wire:key="{{ $key }}">
    ...
    @if (array_key_exists('options', $field))
        ...
        <div class="flex items-center mb-3">
            <input type="radio" wire:model="form.{{ $field['handle'] }}" id="{{ $key }}" value="{{ $key }}" class="w-4 h-4 border-line accent-terracotta focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-terracotta">
